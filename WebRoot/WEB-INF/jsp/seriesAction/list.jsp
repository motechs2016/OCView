...
<div class="container">
    <div class="location">当前位置：基本管理 -&gt;系列管理</div>
    
    <div class="blank10"></div>
    
    <%-- start sturts2 tag(s:form) --%>
	<s:form action="series_delete" method="post">
    <div class="block">
        <div class="h">
            ...
            <div class="bar">
                ...
                <s:a cssClass="btn-lit" action="series_addUI"><span>新增</span></s:a>
            	<s:if test="#session.admin.hasPrivilegeByUrl(\"/backend/series_delete\")">
        ...
        <div class="cnt-wp">
                    ...
                    <tbody>
			        	...
			        	<tr>
			        		<td class="chk">
			        			...
			        			<%-- 判断:有删除权限 --%>
				        		<s:if test="#session.admin.hasPrivilegeByUrl(\"/backend/series_delete\")">
				        			<input type="checkbox" name="seriesIds" value="${seriesId}" id="cb_${seriesId}" />
				        		</s:if>
						    	<s:else>
				        			<input type="checkbox" class="checkbox_disabled" disabled="disabled"/>
						    	</s:else>
			        		</td>
					    	<td class="txt160 c">${seriesName}</td>
					    	<td class="txt160 c">${seriesInfo}</td>
					    	<td class="txt160 c">${seriesDescription}</td>
					    	<td class="icon"><s:a cssClass="opt" title="修改" action="series_editUI?seriesId=%{seriesId}"><span  class="icon-sprite icon-edit"></span></s:a></td>
					    	<td class="icon tail"><s:a cssClass="opt" title="删除" action="series_delete?seriesIds=%{seriesId}" onClick="return window.confirm('您确定要删除？')"><span class="icon-sprite icon-delete"></span></s:a></td>
					    </tr>
					    </s:iterator>   
            ...
            <!-- 分页导航   -->            
            <script type="text/javascript">
                var pagerUrl = 'backend/series_list?pageNum={0}';
