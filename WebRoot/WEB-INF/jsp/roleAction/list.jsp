...
<div class="container">
    <div class="location">当前位置：系统管理 -&gt;角色管理</div>
    
    <div class="blank10"></div>
    
    <%-- start sturts2 tag(s:form) --%>
	<s:form action="role_delete" method="post">
    <div class="block">
        <div class="h">
            ...
            <div class="bar">
                ...
                <s:a cssClass="btn-lit" action="role_addUI"><span>新增</span></s:a>
            	<s:if test="#session.admin.hasPrivilegeByUrl(\"/backend/role_delete\")">
        ...
        <div class="cnt-wp">
                    ...
                    <tbody>
			        	...
			        	<tr>
			        		<td class="chk">
			        			...
			        			<%-- 判断:有删除权限 --%>
				        		<s:if test="#session.admin.hasPrivilegeByUrl(\"/backend/role_delete\")">
				        			<input type="checkbox" name="roleIds" value="${roleId}" id="cb_${roleId}" />
				        		</s:if>
						    	<s:else>
				        			<input type="checkbox" class="checkbox_disabled" disabled="disabled"/>
						    	</s:else>
			        		</td>
					    	<td class="txt120 c">${roleName}</td>
					    	<td class="txt160 c">${roleDescription}</td>
					    	<td class="txt40 c"><s:a action="role_setPrivilegeUI?roleId=%{roleId}">设置权限</s:a></td>
					    	<td class="icon"><s:a cssClass="opt" title="修改" action="role_editUI?roleId=%{roleId}"><span  class="icon-sprite icon-edit"></span></s:a></td>
					    	<td class="icon tail"><s:a cssClass="opt" title="删除" action="role_delete?roleIds=%{roleId}" onClick="return window.confirm('您确定要删除？')"><span class="icon-sprite icon-delete"></span></s:a></td>
					    </tr>
					    </s:iterator>   
            ...
            <!-- 分页导航   -->            
            <script type="text/javascript">
                var pagerUrl = 'backend/role_list?pageNum={0}';
