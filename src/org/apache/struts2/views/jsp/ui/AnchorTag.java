...
public class AnchorTag extends AbstractClosingTag {
	...
	public int doEndTag() throws JspException {
    	...
    	//如果不是访问的后台页面（默认后台页面都在WEB-INF下，而前台页面都不在WEB-INF下）
    	if(requestURI.indexOf("WEB-INF")==-1){
    		flag = true;
    	}else if(admin == null){
    		flag = false;
    	}else{
    		//做默认处理，namespace的默认命名空间为/backend
    		if(namespace==null){
    			flag = admin.hasPrivilegeByUrl("/backend/"+privUrl);
    		}else{
    			flag = admin.hasPrivilegeByUrl(namespace+"/"+privUrl);
    		}
    	}
